...
final class AcronymTests: XCTestCase {
    ...
    func testAcronymCanBeSavedWithApi() throws {
        ...
        let createAcronymData = CreateAcronymData(
            short: acronymShort,
            long: acronymLong)
        ...
        try app.test(.POST, acronymsURI, loggedInUser: user, beforeRequest: { request in
            ...
            try app.test(.GET, acronymsURI, afterResponse: { allAcronymsReponse in
                let acronyms = try allAcronymsReponse.content.decode([Acronym].self)
    ...
    func testUpdatingAnAcronym() throws {
        ...
        try app.test(.GET, "\(acronymsURI)\(acronym.id!)", afterResponse: { response in
            ...
            XCTAssertEqual(acronym.short, acronymShort)
            XCTAssertEqual(acronym.long, newLong)
            XCTAssertEqual(acronym.$user.id, newUser.id)
        })
    }
